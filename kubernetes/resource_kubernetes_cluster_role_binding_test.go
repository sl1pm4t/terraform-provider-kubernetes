...
func TestAccKubernetesClusterRoleBinding_basic(t *testing.T) {
	...
	name := fmt.Sprintf("tf-crb-test-%s", acctest.RandStringFromCharSet(10, acctest.CharSetAlphaNum))
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttr("kubernetes_cluster_role_binding.test", "metadata.0.annotations.TestAnnotationOne", "one"),
					resource.TestCheckResourceAttr("kubernetes_cluster_role_binding.test", "metadata.0.annotations.%", "2"),
					resource.TestCheckResourceAttr("kubernetes_cluster_role_binding.test", "metadata.0.labels.%", "3"),
					resource.TestCheckResourceAttr("kubernetes_cluster_role_binding.test", "metadata.0.labels.TestLabelOne", "one"),
					resource.TestCheckResourceAttr("kubernetes_cluster_role_binding.test", "metadata.0.labels.TestLabelThree", "three"),
					...
					resource.TestCheckResourceAttr("kubernetes_cluster_role_binding.test", "subject.0.name", "test"),
				),
			},
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttr("kubernetes_cluster_role_binding.test", "metadata.0.annotations.%", "2"),
					resource.TestCheckResourceAttr("kubernetes_cluster_role_binding.test", "metadata.0.annotations.TestAnnotationOne", "one"),
					resource.TestCheckResourceAttr("kubernetes_cluster_role_binding.test", "metadata.0.annotations.Different", "1234"),
					testAccCheckMetaAnnotations(&conf.ObjectMeta, map[string]string{"TestAnnotationOne": "one", "Different": "1234"}),
					resource.TestCheckResourceAttr("kubernetes_cluster_role_binding.test", "metadata.0.labels.%", "2"),
					resource.TestCheckResourceAttr("kubernetes_cluster_role_binding.test", "metadata.0.labels.TestLabelOne", "one"),
					resource.TestCheckResourceAttr("kubernetes_cluster_role_binding.test", "metadata.0.labels.TestLabelThree", "three"),
					...
					resource.TestCheckResourceAttrSet("kubernetes_cluster_role_binding.test", "metadata.0.uid"),
					resource.TestCheckResourceAttr("kubernetes_cluster_role_binding.test", "subject.0.name", "test"),
					resource.TestCheckResourceAttr("kubernetes_cluster_role_binding.test", "subject.1.name", "test-two"),
					resource.TestCheckResourceAttr("kubernetes_cluster_role_binding.test", "role_ref.0.name", "test"),
				),
			},
		},
	})
...
func testAccCheckKubernetesClusterRoleBindingExists(n string, obj *rbac.ClusterRoleBinding) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		out, err := conn.RbacV1beta1().ClusterRoleBindings().Get(name, meta_v1.GetOptions{})
		if err != nil {
			return err
		}
